...
**Problem.** The Inbox is great for "things to act on" — bite-sized
items each with a click action. It's NOT great for "what did I work
on this week" — that's a multi-source summary the user reads, not a
list they act on.

The Inbox tells you what's next. A briefing tells you what just
happened (or what's about to).

**Proposal.** A new top-level concept: **Briefings**. Each briefing
is a markdown file at \`~/.jarvis/briefings/<name>/<date>.md\`,
generated by a skill that pulls from multiple sources.

Built-in briefings:

- **Daily recap** (morning): yesterday's meetings (Summary sections),
  PRs merged + opened, Linear issues moved, Slack threads concluded.
  Sources cited inline as markdown links.
- **Weekly retro** (Monday morning): last 7 days, same shape but
  themed — "what I shipped", "what blocked me", "open threads".
- **Today's focus** (morning): upcoming calendar events, PRs awaiting
  review, scheduled actions firing today. With proposed priority.
- **Project digest** (per-project, weekly): just that one project's
  activity, useful when juggling 4-5 projects.

**Sketch.**

1. New top-level tab **Briefings** (or merge into Dashboard).
2. Each briefing is a SKILL.md that pulls from MCPs + ~/.jarvis +
   tasks DB and Writes a markdown file. Schedule via routines (like
   any other skill).
3. The view lists briefings by date, click → render markdown with
   citation links that open the source (meeting transcript, PR, Slack
   thread, …).
4. Briefings can reference inbox items inline: "you have 3 PRs to
   review — see Inbox" links back.

**Effort.** ~2-3 days for the view + 3-4 built-in briefing skills.
The briefing skills are mostly prompt engineering. The view is small.

**Why it matters.** The user's specific ask was "summary of what I
worked on yesterday / last week, what I'm going to work on today /
next week, with references." That's exactly this. The Inbox handles
the "act now" side; Briefings handles the "stay oriented" side.
